...
const BlindsIntervalSlider = props => {
  const sliderStepsProps = {
    ...
    timeNumberOfSteps:
      (props.MaximumValue - props.MinimumValue) / props.ValueIncrements + 1,
  };
  ...
  return (
    <View>
      <View style={styles.sliderNumberContainer}>
        {LengthsList.timeStepList.map((line, index) => (
          <Text
            key={index}
            style={
              props.raiseBlindInterval === line
                ? styles.sliderNumberSelected
                : styles.sliderNumber
            }>
            {line}m
          </Text>
